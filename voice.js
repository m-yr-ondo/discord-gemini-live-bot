    ...
    receiver.speaking.on('start', async (speakingUserId) => {
        ...
        if (guild) {
            ...
            if (firstTextChannel) {
                ...
                console.warn(`[Start] Could not find a suitable text channel for feedback in guild ${guild.name}.`);
            }
        } else {
            console.warn(`[Start] Could not find guild for voice connection ${connection.joinConfig.guildId}.`);
        }
...
function cleanupAndReset(isError = false) {
    ...
    if (textChannelForCleanup) {
        const statusMessage = isError ? "Processing finished with an error. I'm ready to listen again." : "I'm ready to listen again.";
